...
void CodeGenListener::exitBooleanExpr(AslParser::BooleanExprContext *ctx) {
    ...
    std::string label = codeCounters.newLabelIF();
    if (ctx->AND()) {
        std::string labelFalse = "false" + label;
        code = code1 || instruction::LOAD(temp, addr1) || instruction::FJUMP(addr1, labelFalse) 
            || code2 || instruction::LOAD(temp, addr2) || instruction::LABEL(labelFalse); 
    }
    else {
        std::string labelTrue = "true" + label;
        code = code1 || instruction::NOT(temp, addr1) || instruction::FJUMP(temp, labelTrue)
            || code2 || instruction::LOAD(temp, addr2) || instruction::LABEL(labelTrue);
    }
    putAddrDecor(ctx, temp);
    putOffsetDecor(ctx, "");
    putCodeDecor(ctx, code);*/
...
// Modificar només aquestes dos per arrayss multidimensionals
...
void CodeGenListener::enterRepeatStmt(AslParser::RepeatStmtContext *ctx) {
    DEBUG_ENTER();
}
void CodeGenListener::exitRepeatStmt(AslParser::RepeatStmtContext *ctx) {
    instructionList   code;
    std::string      addrBoolCond = getAddrDecor(ctx->expr());
    instructionList  codeBoolCond = getCodeDecor(ctx->expr());
    instructionList  codeRepeat = getCodeDecor(ctx->statements());
    std::string labelRepeat = "repeat" + codeCounters.newLabelWHILE();
    code = instruction::LABEL(labelRepeat) || codeRepeat || 
            codeBoolCond || instruction::FJUMP(addrBoolCond, labelRepeat);
    putCodeDecor(ctx, code);
    DEBUG_EXIT();
}

void CodeGenListener::enterMultiAssignStmt(AslParser::MultiAssignStmtContext *ctx) {
    DEBUG_ENTER();
}
void CodeGenListener::exitMultiAssignStmt(AslParser::MultiAssignStmtContext *ctx) {
    instructionList code, code1, code2;
    unsigned int size = std::min(ctx->multi_id()->left_expr().size(), ctx->multi_expr()->expr().size());
    TypesMgr::TypeId t1, t2, t3;
    std::vector <std::string> temporals = std::vector <std::string> (size);
    for (unsigned int i = 0;i < size;++i) {
        auto expr_i = ctx->multi_expr()->expr(i);
        code2 = getCodeDecor(expr_i);
        temporals[i] = "%" + codeCounters.newTEMP();
        code = code || code2;
        code = code || instruction::LOAD(temporals[i], getAddrDecor(expr_i));
    }
    for (unsigned int i = 0;i < size;++i) {
        auto expr_i = ctx->multi_expr()->expr(i);
        auto ident_i = ctx->multi_id()->left_expr(i);
        t1 = getTypeDecor(ident_i);
        t2 = getTypeDecor(ident_i->ident_refer()->ident());
        t3 = getTypeDecor(expr_i);
        std::string addr1 = getAddrDecor(ident_i);
        std::string addr2 = temporals[i];
        std::string offs1 = getOffsetDecor(ident_i);
        ...
        code = code || getCodeDecor(ident_i);
...
void CodeGenListener::enterPowerExpr(AslParser::PowerExprContext *ctx) {
    DEBUG_ENTER();
}
void CodeGenListener::exitPowerExpr(AslParser::PowerExprContext *ctx) {

    std::string addrB = getAddrDecor(ctx->expr(0));
    instructionList codeB = getCodeDecor(ctx->expr(0));
    TypesMgr::TypeId t1 = getTypeDecor(ctx->expr(0));

    std::string addrE = getAddrDecor(ctx->expr(1));
    instructionList codeE = getCodeDecor(ctx->expr(1));

    std::string result = "%" + codeCounters.newTEMP();
    std::string zero = "%" + codeCounters.newTEMP();
    std::string decrement = "%" + codeCounters.newTEMP();
    std::string boolComp = "%" + codeCounters.newTEMP();
    std::string label = codeCounters.newLabelWHILE();
    std::string labelWhile = "while" + label;
    std::string labelEndWhile = "endwhile" + label;

    instructionList code = codeB;
    if (Types.isIntegerTy(t1)) {
        std::string addrB2 = addrB;
        addrB = "%" + codeCounters.newTEMP();
        code = code || instruction::FLOAT(addrB, addrB2);
    }

    std::string tempB = "%" + codeCounters.newTEMP();
    std::string tempE = "%" + codeCounters.newTEMP();
    std::string temp = "%" + codeCounters.newTEMP();
    std::string labelEndIf = "endif" + codeCounters.newLabelIF();
    code = code || codeE || instruction::LOAD(tempB, addrB) || instruction::LOAD(tempE, addrE)
        || instruction::ILOAD(zero, "0") || instruction::LT(boolComp, tempE, zero)
        || instruction::FJUMP(boolComp, labelEndIf) || instruction::FLOAD(temp, "1.0")
        || instruction::FDIV(tempB, temp, tempB) || instruction::NEG(tempE, tempE)
        || instruction::LABEL(labelEndIf);
        
        
    code = code || instruction::FLOAD(result, "1.0")
        || instruction::LABEL(labelWhile) || instruction::LT(boolComp, zero, tempE)
        || instruction::FJUMP(boolComp, labelEndWhile) || instruction::FMUL(result, result, tempB) 
        || instruction::ILOAD(decrement, "1") || instruction::SUB(tempE, tempE, decrement) 
...
void CodeGenListener::enterSwitchStmt(AslParser::SwitchStmtContext *ctx) {
    DEBUG_ENTER();
}
void CodeGenListener::exitSwitchStmt(AslParser::SwitchStmtContext *ctx) {
    instructionList code = getCodeDecor(ctx->expr());
    std::string addr1 = getAddrDecor(ctx->expr());
    std::string addr_val;
    instructionList code_val, code_stmts;
    std::string labelSwitch = "SwitchCase";
    std::string labelEnd = labelSwitch + "End";
    std::string labelDefault = labelSwitch + "Default";
    std::string labelSwitchCase;

    std::string temp = "%" + codeCounters.newTEMP();
    unsigned int size = ctx->caseVal().size();
    bool thereisDefault = ctx->defaultCase();

    for (unsigned int i = 0;i < size; ++i) {
        auto case_i = ctx->caseVal(i);
        auto val_i = case_i->value();
        code_val = getCodeDecor(val_i);
        addr_val = getAddrDecor(val_i);
        code_stmts = getCodeDecor(case_i->statements());

        if (i > 0) {
            labelSwitchCase = labelSwitch + val_i->getText();
            code = code || instruction::LABEL(labelSwitchCase);
        }

        code = code || code_val || instruction::EQ(temp, addr1, addr_val);

        if (i < size - 1) {
            auto val_i_next = ctx->caseVal(i+1)->value();
            labelSwitchCase = labelSwitch + val_i_next->getText();
            code = code || instruction::FJUMP(temp, labelSwitchCase) 
                || code_stmts || instruction::UJUMP(labelEnd);
        }
        else {
            if (thereisDefault) {
                code = code || instruction::FJUMP(temp, labelDefault)
                    || code_stmts || instruction::UJUMP(labelEnd);
            }
            else {
                code = code || instruction::FJUMP(temp, labelEnd)
                    || code_stmts;
            }
        }
    }
    if (thereisDefault) {
        code = code || instruction::LABEL(labelDefault) 
            || getCodeDecor(ctx->defaultCase()->statements());
    }
