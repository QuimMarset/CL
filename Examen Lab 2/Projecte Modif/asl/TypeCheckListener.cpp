...
/*void TypeCheckListener::exitIdent_refer(AslParser::Ident_referContext *ctx) {
    ...
    if (size > 0) {
        if (not Types.isErrorTy(t1) and not Types.isArrayTy(t1)) {
            ...
            unsigned int i = 0;
            while (Types.isArrayTy(t1) and i < size) {
                t1 = Types.getArrayElemType(t1);
                i += 1;
            }
            if (size > i) {
                std::cout << "Error: No hi ha tantes dimensions" << std::endl;
                t1 = Types.createErrorTy();
            }
        }
        TypesMgr::TypeId tIndex;
        for (auto expr_i : ctx->expr()) {
            tIndex = getTypeDecor(expr_i);
            if (not Types.isIntegerTy(tIndex)) {
                Errors.nonIntegerIndexInArrayAccess(expr_i);
            }
        }
    }
    putTypeDecor(ctx, t1);
...
void TypeCheckListener::enterRepeatStmt(AslParser::RepeatStmtContext *ctx) {
    DEBUG_ENTER();
}
void TypeCheckListener::exitRepeatStmt(AslParser::RepeatStmtContext *ctx) {
    TypesMgr::TypeId t1 = getTypeDecor(ctx->expr());
    if (not Types.isErrorTy(t1) and not Types.isBooleanTy(t1)) {
        Errors.booleanRequired(ctx);
    }
...
void TypeCheckListener::enterMultiAssignStmt(AslParser::MultiAssignStmtContext *ctx) {
    DEBUG_ENTER();
}
void TypeCheckListener::exitMultiAssignStmt(AslParser::MultiAssignStmtContext *ctx) {

    unsigned int ident_number = ctx->multi_id()->left_expr().size();
    unsigned int expr_number = ctx->multi_expr()->expr().size(); 
    if (ident_number > expr_number) {
        std::cout << ctx->getStart()->getLine() << "Faltan expresiones" << std::endl;
    }
    else if (ident_number < expr_number) {
        std::cout << ctx->getStart()->getLine() << "Faltan identificadores" << std::endl;
    }
    unsigned int min = std::min(ident_number, expr_number);
    for (unsigned int i = 0;i < min; ++i) {
        auto ident_i = ctx->multi_id()->left_expr(i);
        auto expr_i = ctx->multi_expr()->expr(i);
        TypesMgr::TypeId t1 = getTypeDecor(ident_i);
        TypesMgr::TypeId t2 = getTypeDecor(expr_i);
        if (not Types.isErrorTy(t1) and not Types.isErrorTy(t2) and not Types.copyableTypes(t1, t2)) {
            std::cout << ctx->getStart()->getLine() << " Assignacion con tipos incompatibles" << std::endl;
        }
        if (not Types.isErrorTy(t1) and not getIsLValueDecor(ident_i)) {
            std::cout << ctx->getStart()->getLine() << " Left-expr no referenciable" << std::endl;
        }
    }
}

void TypeCheckListener::enterPowerExpr(AslParser::PowerExprContext *ctx) {
    DEBUG_ENTER();
}
void TypeCheckListener::exitPowerExpr(AslParser::PowerExprContext *ctx) {
    TypesMgr::TypeId t1 = getTypeDecor(ctx->expr(0));
    TypesMgr::TypeId t2 = getTypeDecor(ctx->expr(1));
    if (not Types.isErrorTy(t1) and not Types.isNumericTy(t1)) {
        std::cout << ctx->getStart()->getLine() << "Potència amb base no numèrica" << std::endl;
    }
    if (not Types.isErrorTy(t2) and not Types.isIntegerTy(t2)) {
        std::cout << ctx->getStart()->getLine() << "Potència amb exponent no enter" << std::endl;
    }
    TypesMgr::TypeId t3 = Types.createFloatTy();
    putTypeDecor(ctx, t3);
    putIsLValueDecor(ctx, false);
    DEBUG_EXIT();
}

void TypeCheckListener::enterSwitchStmt(AslParser::SwitchStmtContext *ctx) {
    DEBUG_ENTER();
}
void TypeCheckListener::exitSwitchStmt(AslParser::SwitchStmtContext *ctx) {
    TypesMgr::TypeId t1 = getTypeDecor(ctx->expr());
    TypesMgr::TypeId t2;
    for (auto case_i : ctx->caseVal()) {
        t2 = getTypeDecor(case_i->value());
        if (not Types.isErrorTy(t1) and not Types.isErrorTy(t2) and not 
            Types.copyableTypes(t1, t2)) {
                Errors.incompatibleAssignment(ctx->SWITCH());
                break;
        }
    }
    DEBUG_EXIT();
